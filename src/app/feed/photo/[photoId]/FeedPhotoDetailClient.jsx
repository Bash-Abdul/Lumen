// app/feed/photo/[photoId]/FeedPhotoDetailClient.js
"use client";
...
export default function FeedPhotoDetailClient({ post, galleryIds = [] }) {
  ...
  const handleNavigation = (photoId) => {
    const galleryParam = galleryIds.length > 0 ? `?gallery=${galleryIds.join(',')}` : '';
    router.push(`/feed/photo/${photoId}${galleryParam}`);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex">
      ...
      <button
        onClick={handleClose}
        className="absolute top-4 right-4 z-60 w-12 h-12 flex items-center justify-center rounded-full bg-zinc-900 hover:bg-zinc-800 text-white transition-colors border border-zinc-700 shadow-lg"
        aria-label="Close"
      >
        <svg
          className="w-6 h-6"
