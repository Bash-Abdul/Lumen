...
// export default function PostCard({ post, allPostIds = [] }) {
...
//   //   // const galleryParam = allPostIds.length > 0 ? `&gallery=${allPostIds.join(',')}` : '';
//   //   // router.push(`/${post.username}/media/${post.id}?from=feed${galleryParam}`);
//   //   router.push(`/${post.username}/media/${post.id}`);
//   // };
//   // const handlePostClick = () => {
//   //   const galleryParam = allPostIds.length > 0 ? `?gallery=${allPostIds.join(',')}` : '';
//   //   router.push(`/feed/photo/${post.id}${galleryParam}`);
//   // };
//   const handlePostClick = () => {
//     router.push(`/${post.username}/media/${post.id}`); // ✅ Fixed: now uses /[username]/media/[photoId]
//   };
...
export default function PostCard({ post, allPosts = [] }) {
  const router = useRouter();
  ...
  // Build gallery in the exact format PhotoDetailClient expects: "id:username,id:username,..."
  // We prefer allPosts if provided, else fallback to just current post.
  const gallery = useMemo(() => {
    const src = Array.isArray(allPosts) && allPosts.length ? allPosts : [post];
    return src
      .map((p) => `${p.id}:${p.username}`)
      .join(",");
  }, [allPosts, post]);

  // Optional but nice: preserve the exact feed URL (tab type etc.) so direct-entry fallback works too
  ...
  const handlePostClick = () => {
    ...
    if (gallery) sp.set("gallery", gallery);

    router.push(`/${post.username}/media/${post.id}?${sp.toString()}`);
  };
  ...
  return (
    <article className="relative overflow-hidden border border-zinc-800 bg-zinc-950/60 group break-inside-avoid">
      ...
      <div
        className="absolute inset-0 bg-linear-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200"
        onClick={handlePostClick}
      />
